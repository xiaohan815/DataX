...
PanweiWriter插件实现了写入数据到 PanWei主库目的表的功能。在底层实现上，PanweiWriter通过JDBC连接远程 PanWei 数据库，并执行相应的 insert into ... sql 语句将数据写入 PanWei，内部会分批次提交入库。

PanweiWriter面向ETL开发工程师，他们使用PanweiWriter从数仓导入数据到PanWei。同时 PanweiWriter亦可以作为数据迁移工具为DBA等用户提供服务。


## 2 实现原理

PanweiWriter通过 DataX 框架获取 Reader 生成的协议数据，根据你配置生成相应的SQL插入语句
...
<br />
    ...
    2. PanweiWriter和MysqlWriter不同，不支持配置writeMode参数。
...
* 这里使用一份从内存产生到 PanweiWriter导入的数据。
        ...
        "content": [
            {
                ...
                "writer": {
                    ...
                    "parameter": {
                        ...
                        "connection": [
                            {
                                "jdbcUrl": "jdbc:panwei://127.0.0.1:3002/datax",
                                "table": [
                                    "test"
                                ]
                            }
...
* **jdbcUrl**
    ...
    * 描述：目的数据库的 JDBC 连接信息 ,jdbcUrl必须包含在connection配置单元中。
      ...
      注意：1、在一个数据库上只能配置一个值。
      2、jdbcUrl按照PanWei官方规范，并可以填写连接附加参数信息。具体请参看PanWei官方文档或者咨询对应 DBA。
...
目前 PanweiWriter支持大部分 PanWei类型，但也存在部分没有支持的情况，请注意检查你的类型。

下面列出 PanweiWriter针对 PanWei类型转换列表:
...
**Q: PanweiWriter 执行 postSql 语句报错，那么数据导入到目标数据库了吗?**
