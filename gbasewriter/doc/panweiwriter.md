...
gbaseWriter插件实现了写入数据到 gbase主库目的表的功能。在底层实现上，gbaseWriter通过JDBC连接远程 gbase 数据库，并执行相应的 insert into ... sql 语句将数据写入 gbase，内部会分批次提交入库。

gbaseWriter面向ETL开发工程师，他们使用gbaseWriter从数仓导入数据到gbase。同时 gbaseWriter亦可以作为数据迁移工具为DBA等用户提供服务。


## 2 实现原理

gbaseWriter通过 DataX 框架获取 Reader 生成的协议数据，根据你配置生成相应的SQL插入语句
...
<br />
    ...
    2. gbaseWriter和MysqlWriter不同，不支持配置writeMode参数。
        ...
        "content": [
            {
                ...
                "writer": {
                    ...
                    "parameter": {
                        ...
                        "connection": [
                            {
                                "jdbcUrl": "jdbc:gbase://127.0.0.1:3002/datax",
                                "table": [
                                    "test"
                                ]
                            }
...
* **jdbcUrl**
    ...
    * 描述：目的数据库的 JDBC 连接信息 ,jdbcUrl必须包含在connection配置单元中。
      ...
      注意：1、在一个数据库上只能配置一个值。
      2、jdbcUrl按照gbase官方规范，并可以填写连接附加参数信息。具体请参看gbase官方文档或者咨询对应 DBA。
...
目前 gbaseWriter支持大部分 gbase类型，但也存在部分没有支持的情况，请注意检查你的类型。

下面列出 gbaseWriter针对 gbase类型转换列表:
...
**Q: gbaseWriter 执行 postSql 语句报错，那么数据导入到目标数据库了吗?**
