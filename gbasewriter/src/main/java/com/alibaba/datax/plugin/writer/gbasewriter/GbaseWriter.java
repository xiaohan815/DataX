package com.alibaba.datax.plugin.writer.gbasewriter;
...
public class GbaseWriter extends Writer {
	...
	public static class Job extends Writer.Job {
		...
		public void init() {
			...
			// warn：not like mysql, gbase only support insert mode, don't use
			String writeMode = this.originalConfig.getString(Key.WRITE_MODE);
			if (null != writeMode) {
				throw DataXException.asDataXException(DBUtilErrorCode.CONF_ERROR,
					String.format("写入模式(writeMode)配置有误. 因为gbase不支持配置参数项 writeMode: %s, gbase仅使用insert sql 插入数据. 请检查您的配置并作出修改.", writeMode));
			}
